...
classifications %>% 
  ggplot(aes(x = `Name/Id`, y = prob, fill = response)) +
  geom_col() +
  coord_flip()
```
...
train_validation <- classifications %>%
  ...
  mutate(causa_rechazo = fct_infreq((as_factor(causa_rechazo))) ,
         response = fct_infreq(as_factor(response)))
```
...
60% Accuracy (TP + TN) / Total is quite a good measure for a first try.
